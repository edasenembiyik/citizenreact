...
export const getCitizens = createAsyncThunk(
    ...
    async (search) => {
        try {
            const response = await axios.get(citizenService.getCitizens, {params:{name:search.name,isCitizen:search.isCitizen,numberOfChildren:search.numberOfChildren,hasDrivingLicense:search.hasDrivingLicense}});
